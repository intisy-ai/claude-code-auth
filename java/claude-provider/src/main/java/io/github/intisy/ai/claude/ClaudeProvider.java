...
/**
 * The JVM {@code ClaudeProvider}: {@link #handleIr} runs every request through
 ...
 *
 * <p>IR-native only: the front-door (Router/proxy server) owns app&lt;-&gt;IR translation, so this
 * provider carries ZERO app-wire format code, and the class inherits {@link Provider}'s throwing
 ...
 *
 * <p>Typed capability SPI: the {@code GET/PUT /v1/config}, {@code GET /v1/models}, {@code
 ...
 * methods below, each a thin delegate to {@link ClaudeConfig}/{@link ClaudeModelsFetch}/
 * {@link ClaudeUsageFetch}/{@link ClaudeOAuth} (no duplicated logic). This is JVM/server-facing
 * only; the TeaVM JS export surface ({@code ClaudeProviderJs}) never references this class, and
 * the TS driver keeps its own native config/models/quota/oauth paths.
 */
public final class ClaudeProvider implements Provider, ConfigurableProvider, ModelCatalogProvider,
    ...
    /**
     * The IR-native serving path: encodes the inbound {@link IrRequest} to Anthropic wire text and
     * runs it through the {@link ClaudeHandleOrchestrator} flow (retry/rotation/rate-limit
     ...
     * throw {@link HandleIrException} instead, core-proxy's canonical typed transport error,
     * carrying the real status/headers/body: {@code Router.route} catches it and reconstructs an
     * equivalent {@code HttpResponse}, running it through the SAME rate-limit/fallback logic
     * instead of collapsing to a flat 502 (see {@code HandleIrException}'s own javadoc).
     *
     * <p>The JVM {@link ClaudeHostSeams.HostAttemptExecutor} always fully reads the upstream body
     * into a {@code String} (no true SSE streaming on this path -- {@code ClaudeProviderTest}
    ...
    private static IrResponse decodeServedIrResponse(ClaudeHandleOrchestrator.HandleDecision decision,
        ...
        if (decision.kind == ClaudeHandleOrchestrator.HandleDecision.Kind.SERVE
            ...
            // status/headers/body through the canonical typed transport error so
